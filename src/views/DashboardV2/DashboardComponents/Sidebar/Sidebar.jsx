		...
		if (x.count > 0) {
			return (
				<div className="figure">
					...
					{x.count < 31 ? <p className="count">{x.count}</p> : <p className="count">30+</p>}
				</div>
			);
		}
	};
		...
		<section className="dashboard-sidebar section">
			<div className="dashboard-section-inner-container ">
				<div className="sidebar-top">
					<Link to="/settings" origination="userDashboard">
						<h6>EDIT</h6>
					</Link>
					{user.profileImage ? (
				...
				<div className="dashboard-stats">
					{(
						<div className="a-container">
							{totalAchievements === 0 ? (
								<p>
									You currently have no achievements. Look <NavLink to="/projects">here</NavLink> for
									projects to join, or <NavLink to="/createproject">create a project</NavLink>!
								</p>
							) : (
								countArray.map(y => {
									return (
										<React.Fragment key={y.name + Math.random()}>{displayCount(y)}</React.Fragment>
									);
								})
							)}
						</div>
								...
								<>
									...
									<div className="info project-details-container">
										...
										{project.donations && (
											<div className="text">
												<p>Donations</p>
												<p className="text-value">{project.donations.length}</p>
											</div>
										)}
										{project.applicants && (
											<div className="text">
												<p>Applicants</p>
												<p className="text-value">{project.applicants.length}</p>
											</div>
										)}
										{project.students && (
											<div className="text">
												<p>Students</p>
												<p className="text-value">{project.students.length}</p>
											</div>
										)}
										{project.tradeMasters && (
											<div className="text">
												<p>Trade Masters</p>
												<p className="text-value">{project.tradeMasters.length}</p>
											</div>
										)}
										{project.trades && (
											<div className="text">
												<p>Trades</p>
												<p className="text-value">{project.trades.length}</p>
											</div>
										)}
										{project.tasks && (
											<div className="text">
												<p>Tasks</p>
												<p className="text-value">{project.tasks.length}</p>
											</div>
										)}
										<div className="text">
											...
											<p className="text-value">
												{calculatePercentageProgressBar(
													project.goalAmount,
													addUpDonations(project.donations),
												)}
											</p>
										</div>
										<ProgressBar
