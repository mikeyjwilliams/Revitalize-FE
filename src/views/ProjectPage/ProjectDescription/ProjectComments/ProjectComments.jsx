...
const ProjectComments = ({ comments, refetch, id, history, match, newBool, boolState, deleteBool, setDeleteBool }) => {
	...
	const [addComment] = useMutation(ADD_COMMENT, {
		update(cache, { data: createProjectComment }) {
			...
			cache.writeQuery({
				query: GET_PROJECT_BY_SLUG,
				data: {
					projectBySlug: (projectBySlug.comments =
						createProjectComment.createProjectComment.project.comments),
				},
			});
			newBool(!boolState);
		},
	...
	const [editCommentMutation] = useMutation(EDIT_COMMENT, {
		update(cache, { data: updateProjectComment }) {
			...
			cache.writeQuery({
				...
				data: {
					projectBySlug: (projectBySlug.comments =
						updateProjectComment.updateProjectComment.project.comments),
				},
			});
			newBool(!boolState);
		},
	...
	const settingsBlur = e => {
		if (
			e.target.className === 'settings-drop' ||
			e.target.className === 'dot-container' ||
			...
			{localStorage.getItem('token') && (
				<form className="comment-form click" onSubmit={submitComment}>
					...
					<button
						...
						style={comment.comment.length === 0 ? { background: '#4840ba', cursor: 'default' } : null}
					>
						Submit
					</button>
				</form>
